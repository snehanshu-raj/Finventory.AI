    ...
    <div className="min-h-screen bg-[var(--color-bg)] flex items-center justify-center p-4">
      <div className="w-full max-w-2xl">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[var(--color-muted)]">Set up your smart pantry in a few easy steps</p>
        </div>
        ...
        <div className="flex items-center justify-between mb-8 px-4">
          {steps.map((s, i) => (
            <div key={s.label} className="flex items-center">
              <div className={`flex items-center gap-1.5 ${i <= step ? 'text-sky-400' : 'text-[var(--color-muted)]'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-all
                  ${i < step ? 'bg-sky-500 text-white' : i === step ? 'bg-sky-500/20 text-sky-400 ring-2 ring-sky-400' : 'bg-[var(--color-surface-2)]'}`}>
                  {i < step ? <Check size={16} /> : i + 1}
                </div>
                <span className="hidden md:block text-xs font-medium">{s.label}</span>
              </div>
              {i < steps.length - 1 && (
                <div className={`w-8 md:w-16 h-0.5 mx-1 ${i < step ? 'bg-sky-400' : 'bg-[var(--color-surface-2)]'}`} />
              )}
            </div>
          ))}
        </div>
        ...
        {/* Card */}
        <div className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] p-6 md:p-8 shadow-xl">
